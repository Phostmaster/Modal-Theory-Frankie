...
class MemoryCoordinator:
    ...
    def _split_memory_chunks(self, memory_text: str) -> List[str]:
        ...
        raw_chunks = [chunk.strip() for chunk in memory_text.split("\n\n") if chunk.strip()]
        return raw_chunks
    ...
    def persistence_score(self, chunk: str, prompt: str, phase_lock: float = 255.0) -> float:
        ...
        persistence_cues = (
            "stuck", "overwhelmed", "decision", "burnout", "partner",
            "job", "culture", "money", "opportunity", "confused",
            "clarity", "panic", "panicking", "worry", "worried",
        )
    ...
    def retrieve_persistent_memory(self, prompt: str, pv: PrivilegeVector) -> str:
        ...
        scored = [
            (self.persistence_score(chunk, prompt, phase_lock=255.0), chunk)
            for chunk in chunks
        ]
...
class ModalOrchestrator:
    ...
    def analyze_prompt(self, prompt: str) -> PromptDecision:
        ...
        return PromptDecision(task_type, scores, constrained_mode, True, "embedder")
    ...
    def run_prompt(self, prompt: str, forced_mode: Optional[str] = None) -> RunResult:
        ...
        if forced_mode is None:
            ...
            default_mode = max(scores, key=scores.get)
            ...
            scores["memory_persistence_filter"] = 1.0 if memory_context else 0.0
            scores["memory_context_chars"] = float(len(memory_context))
            scores["relic_resonance"] = resonance_score
            scores["dynamic_token_budget"] = float(pv.max_tokens)
            scores["constraint_ok"] = 1.0 if constraint_ok else 0.0
            scores["constraint_default_mode"] = default_mode
            scores["constraint_route"] = constrained_mode

            decision = PromptDecision(task_type, scores, constrained_mode, True, "embedder")
            ...
            scores["phase_tension"] = phase_state.tension
            scores["phase_calm"] = phase_state.calm
            scores["phase_convergence"] = phase_state.convergence
            scores["phase_topic_shift"] = phase_state.topic_shift
            scores["phase_chiral_bias"] = phase_state.chiral_bias
            forced = False
            ...
            scores["relic_resonance"] = 0.0
            scores["dynamic_token_budget"] = float(pv.max_tokens)
            scores["constraint_ok"] = 1.0
            scores["constraint_default_mode"] = forced_mode
            scores["constraint_route"] = forced_mode
        ...
        return RunResult(
            ...
            scores={
                ...
                **pv.__dict__,
                ...
                "relic_resonance": scores.get("relic_resonance", 0.0),
                "dynamic_token_budget": scores.get("dynamic_token_budget", float(pv.max_tokens)),
                "constraint_ok": scores.get("constraint_ok", 1.0),
                "constraint_default_mode": scores.get("constraint_default_mode", mode),
                "constraint_route": scores.get("constraint_route", mode),
            },
