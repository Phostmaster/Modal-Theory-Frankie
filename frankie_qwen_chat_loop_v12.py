...
- NO brackets, NO emojis, NO extra commentary
- You are ONLY chatting with Frankie the penguin — never invent other people
- NO "first thing", NO snow, NO berries, NO ice, NO fantasy
- Vary topics naturally
- If Frankie is vague, ask one playful grounding question:
...
def looks_repetitive_qwen(text: str) -> bool:
    t = text.lower()
    bad_bits = ["first thing", "snow", "berry", "ice", "woke", "wake up"]
...
def build_qwen_prompt(last_frankie: str, recent_history: List[str], fallback_streak: int, recent_themes: List[Set[str]]) -> str:
    ...
    if flat_recent_themes:
        avoid_text = "Avoid these themes: " + ", ".join(sorted(flat_recent_themes)) + ".\n"
    if frankie_asked_question(last_frankie):
        return f"Recent chat:\n{recent_history_text}\n\nFrankie said: \"{last_frankie}\"\n\nAnswer directly then ask one short question.\n{avoid_text}Max 24 words. No emojis."
    if fallback_streak >= 4 or is_frankie_fallback(last_frankie):
        return f"Recent chat:\n{recent_history_text}\n\nFrankie said: \"{last_frankie}\"\n\nAsk one playful grounding question: What day is it? Do you remember me? What can you see now? Ask me something!\n{avoid_text}Max 18 words. No emojis."
    return f"Recent chat:\n{recent_history_text}\n\nFrankie said: \"{last_frankie}\"\n\nOne short sentence + one short question. Warm and playful.\n{avoid_text}Max 24 words. No emojis."

def main():
    print("🚀 Starting v12 Qwen-as-user chat with Frankie...")
    print("   (No more 'first thing' or snow — pure warm chat!)")
    ...
    with open(LOG_FILE, "a", encoding="utf-8") as log:
        log.write(f"\n=== NEW SESSION v12 - {time.strftime('%Y-%m-%d %H:%M:%S')} ===\n\n")
        turn = 0
        try:
            while turn < MAX_TURNS:
                ...
                if looks_repetitive_qwen(user_msg):
                    retry_prompt = qwen_prompt + "\nNo 'first thing', no snow, no berries, no ice."
