...
- NO brackets, NO emojis, NO poetry words (light, breathing, hum, soft, quiet, feel, glow)
- NO repeating the same question style ("one thing", "last thing", "what can you see")
...
def looks_repetitive_qwen(text: str) -> bool:
    t = text.lower()
    bad_bits = ["one thing", "last thing", "first thing", "what can you see", "light", "breathing", "hum", "soft", "quiet", "feel", "glow", "snow", "berry", "ice"]
...
def main():
    print("🚀 Starting v14 Qwen-as-user chat with Frankie...")
    print("   (No repeating questions — pure normal mate chat!)")
    ...
    with open(LOG_FILE, "a", encoding="utf-8") as log:
        log.write(f"\n=== NEW SESSION v14 - {time.strftime('%Y-%m-%d %H:%M:%S')} ===\n\n")
        turn = 0
        try:
            while turn < MAX_TURNS:
                ...
                if looks_repetitive_qwen(user_msg):
                    retry_prompt = qwen_prompt + "\nNo repeating the same question style. No poetry words."
