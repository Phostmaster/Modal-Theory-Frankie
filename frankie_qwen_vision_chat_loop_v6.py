        ...
        try:
            while turn < MAX_TURNS:
                ...
                try:
                    ...
                    # === ULTRA-STRONG RECALL + FORCED DIRECT ANSWER ===
                    final_text = user_text
                    keywords = ["colour", "color", "item", "name", "furniture", "sofa", "table", "chandelier", "room", "carrot", "rose"]
...
- Do not be poetic or vague"""
                    ...
                    # === FALLBACK OVERRIDE (the safety net) ===
                    if is_frankie_fallback(reply) and any(k in user_text.lower() for k in ["colour", "color", "carrot", "rose", "item", "furniture"]):
                        print("⚡ Fallback override triggered — forcing direct answer")
                        force_prompt = f"From this image: {last_vision_desc}\nAnswer Peter's question '{user_text}' in one short factual sentence only."
                        reply = bridge.backend.generate(force_prompt, temperature=0.0, max_tokens=50).strip()
