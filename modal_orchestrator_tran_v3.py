...
class PrivilegeVector:
    ...
    memory_k:             int     # how many memories to retrieve
    memory_emotional_bias: float  # 0.0 → 1.0
    memory_analytic_bias:  float  # 0.0 → 1.0

    # --- v3 tool / verifier controls ---
    allow_tools:          bool
    verifier_budget:      int     # 0 = off, 1 = cheap verifier allowed
...
def compute_privilege_vector(scores: Dict[str, float]) -> PrivilegeVector:
    ...
    # Retrieval becomes graded, not binary
    memory_strength = max(0.0, min(1.0, (1.0 - home)))
    retrieval = memory_strength > 0.25

    # Emotional vs analytic retrieval bias
    ...
    # Tools + verifier
    allow_tools = (structure > 0.62 and home < 0.55)
    verifier_budget = 1 if (structure > 0.72 and home < 0.55) else 0
...
class ModalOrchestrator:
    ...
    def retrieve_memory_context(self, prompt: str, pv: PrivilegeVector) -> str:
        """
        v3 memory retrieval wrapper.
        Uses privilege-controlled retrieval depth and bias.
        Falls back safely if MemoryGate does not yet support weighted args.
        """
        if not pv.retrieval:
            return ""
        ...
        try:
            ...
            # Backward-compatible fallback until MemoryGate is upgraded
    ...
    def build_system_prompt_and_settings(
        ...
        # Light vs strong memory injection
    ...
    def call_local_verifier(self, user_prompt: str, draft_response: str) -> str:
        ...
        input_len = inputs["input_ids"].shape[1]
        verdict = self.local_tokenizer.decode(
    ...
    def call_local_repair(
        ...
        messages = [
            {
                "role": "system",
                "content": (
                    "Revise the draft conservatively. "
        ...
        input_len = inputs["input_ids"].shape[1]
        repaired = self.local_tokenizer.decode(
            output_ids[0][input_len:], skip_special_tokens=True
        ).strip()
    ...
    def maybe_run_tooling(
        ...
        """
        v3 tool/agent gate stub.
        Leave as a no-op for now; later this becomes calculator/verifier/compare tool routing.
        """
        if not pv.allow_tools:
            return response

        # Placeholder: no external tool use yet
        return response
