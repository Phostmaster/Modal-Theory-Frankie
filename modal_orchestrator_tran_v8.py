...
class PhaseStateLogitsProcessor(LogitsProcessor):
    def __init__(
        self,
        tokenizer,
        ...
        self.question_token_ids = tokenizer.encode("?", add_special_tokens=False)
        self.period_token_ids = tokenizer.encode(".", add_special_tokens=False)
        self.newline_token_ids = tokenizer.encode("\n", add_special_tokens=False)
        self.eos_token_id = tokenizer.eos_token_id

    def _apply_bias(self, scores: torch.Tensor, token_ids: List[int], amount: float) -> None:
        for token_id in token_ids:
            if 0 <= token_id < scores.shape[-1]:
                scores[:, token_id] += amount
    ...
    def __call__(self, input_ids: torch.LongTensor, scores: torch.FloatTensor) -> torch.FloatTensor:
        ...
        if self.mode == "engagement":
            ...
            if self.phase_state.tension >= 0.55 and self.phase_state.chiral_bias >= 0.0:
                question_bias += 0.35
                close_bias -= 0.15

            if self.phase_state.convergence >= 0.60:
                question_bias -= 0.20
                close_bias += 0.20

            self._apply_bias(adjusted, self.question_token_ids, question_bias)
            self._apply_bias(adjusted, self.period_token_ids, close_bias * 0.6)
            self._apply_bias(adjusted, self.newline_token_ids, close_bias * 0.4)

            if self.eos_token_id is not None and 0 <= self.eos_token_id < adjusted.shape[-1]:
                adjusted[:, self.eos_token_id] += close_bias

        elif self.mode == "analytic":
            analytic_close_bias = 0.15 + (self.phase_state.calm * 0.10)
            question_down_bias = -0.10

            self._apply_bias(adjusted, self.period_token_ids, analytic_close_bias)
            self._apply_bias(adjusted, self.question_token_ids, question_down_bias)

            if self.eos_token_id is not None and 0 <= self.eos_token_id < adjusted.shape[-1]:
                adjusted[:, self.eos_token_id] += analytic_close_bias * 0.8
...
class ModalOrchestrator:
    def __init__(
        ...
        self._mt_original_attn_forwards = []
        self._mt_attention_bias_tensor = None
        self._mt_attention_bias_active = False

        self._mt_original_prepare_inputs = None
        self._mt_kv_bias_active = False

        self._mt_hidden_state_hook_handles = []
        self._mt_hidden_state_bias_active = False
    ...
    def call_local_model(
        ...
        self.install_attention_phase_bias(phase_state, mode)
        self.install_kv_persistence_hook(phase_state, mode)
        self.install_hidden_state_evolution_hook(phase_state, mode)
        try:
            with torch.inference_mode():
            ...
            self.clear_attention_phase_bias()
            self.clear_kv_persistence_hook()
            self.clear_hidden_state_evolution_hook()
        input_len = inputs["input_ids"].shape[1]
        new_tokens = output_ids[0][input_len:]
    ...
    def build_phase_logits_processor(
        self,
    ...
    def build_attention_phase_bias(
        self,
        phase_state: Optional[PhaseState],
        mode: Optional[str],
        key_length: int,
        ...
        if mode not in ("engagement", "analytic"):
            return None

        if key_length <= 0:
            return None

        positions = torch.linspace(0.0, 1.0, steps=key_length, device=device, dtype=dtype)

        recent_bias = positions.view(1, 1, 1, key_length)
        distant_bias = (1.0 - positions).view(1, 1, 1, key_length)

        persistence_strength = 0.08 + (phase_state.calm * 0.06)
        chiral_strength = 0.05 * phase_state.chiral_bias
        tension_strength = 0.08 * phase_state.tension

        if mode == "engagement":
            bias = (recent_bias * persistence_strength) + chiral_strength - (distant_bias * tension_strength)
        else:
            analytic_focus = 0.05 + (phase_state.calm * 0.04)
            bias = (recent_bias * analytic_focus) - (distant_bias * (0.04 * phase_state.tension))

        return bias

    def compute_kv_persistence_scale(
        self,
        phase_state: Optional[PhaseState],
        mode: Optional[str],
        seq_len: int,
        ...
        recent = positions.view(1, 1, seq_len, 1)
        distant = (1.0 - positions).view(1, 1, seq_len, 1)

        persistence_strength = 0.04 + (phase_state.calm * 0.04)
        tension_penalty = 0.03 * phase_state.tension
        chiral_term = 0.02 * max(0.0, phase_state.chiral_bias)

        if mode == "engagement":
            scale = 1.0 + (recent * (persistence_strength + chiral_term)) - (distant * tension_penalty)
        else:
            analytic_stability = 0.03 + (phase_state.calm * 0.03)
            scale = 1.0 + (recent * analytic_stability) - (distant * (0.02 * phase_state.tension))

        return scale.clamp(0.92, 1.08)

    def apply_kv_persistence(
        self,
        past_key_values,
        phase_state: Optional[PhaseState],
        mode: Optional[str],
        ...
        for layer_kv in past_key_values:
            if not isinstance(layer_kv, (tuple, list)) or len(layer_kv) < 2:
                adjusted_layers.append(layer_kv)
                continue

            key_tensor = layer_kv[0]
            value_tensor = layer_kv[1]
            ...
            scale = self.compute_kv_persistence_scale(
            ...
            new_key = key_tensor * scale
            new_value = value_tensor * scale

            if len(layer_kv) == 2:
                adjusted_layers.append((new_key, new_value))
            else:
                adjusted_layers.append((new_key, new_value, *layer_kv[2:]))

        self._mt_kv_bias_active = True
        return tuple(adjusted_layers)

    def install_kv_persistence_hook(
        self,
        phase_state: Optional[PhaseState],
        mode: Optional[str],
    ) -> None:
        ...
        if self._mt_original_prepare_inputs is not None:
            self.clear_kv_persistence_hook()

        if phase_state is None or mode not in ("engagement", "analytic"):
            return

        original_prepare_inputs = self.local_model.prepare_inputs_for_generation
        self._mt_original_prepare_inputs = original_prepare_inputs
        self._mt_kv_bias_active = False

        def wrapped_prepare_inputs_for_generation(*args, **kwargs):
            model_inputs = original_prepare_inputs(*args, **kwargs)

            if isinstance(model_inputs, dict) and "past_key_values" in model_inputs:
                model_inputs["past_key_values"] = self.apply_kv_persistence(
        ...
        self.local_model.prepare_inputs_for_generation = wrapped_prepare_inputs_for_generation

    def clear_kv_persistence_hook(self) -> None:
        if self.local_model is not None and self._mt_original_prepare_inputs is not None:
            self.local_model.prepare_inputs_for_generation = self._mt_original_prepare_inputs
    ...
    def evolve_hidden_states(
        ...
        deviation = hidden_states - centroid

        tension_penalty = 0.015 * phase_state.tension
        calm_coherence = 0.010 * phase_state.calm

        evolved = hidden_states - (deviation * tension_penalty) + (centroid * calm_coherence)

        if mode == "engagement":
            chiral_term = 0.006 * phase_state.chiral_bias
            evolved = evolved + chiral_term

        elif mode == "analytic":
            convergence_term = 0.006 * phase_state.convergence
            evolved = evolved + (centroid * convergence_term)

        return evolved

    def install_hidden_state_evolution_hook(
        self,
        phase_state: Optional[PhaseState],
        mode: Optional[str],
    ) -> None:
        ...
        base_model = getattr(self.local_model, "model", None)
        ...
        def make_hook():
            def hook(module, inputs, output):
                if isinstance(output, tuple) and len(output) > 0 and torch.is_tensor(output[0]):
                    new_hidden = self.evolve_hidden_states(output[0], phase_state, mode)
                    self._mt_hidden_state_bias_active = True
                    return (new_hidden, *output[1:])

                if torch.is_tensor(output):
                    self._mt_hidden_state_bias_active = True
                    return self.evolve_hidden_states(output, phase_state, mode)
        ...
        for layer in layers:
            ...
            self._mt_hidden_state_hook_handles.append(handle)

    def clear_hidden_state_evolution_hook(self) -> None:
        for handle in self._mt_hidden_state_hook_handles:
            try:
                handle.remove()
            except Exception:
                pass
    ...
    def install_attention_phase_bias(
        self,
        phase_state: Optional[PhaseState],
        mode: Optional[str],
    ) -> None:
        ...
        for layer in layers:
            ...
            self._mt_original_attn_forwards.append((attn, original_forward))

            def wrapped_forward(attn_self, *args, **kwargs):
                attention_mask = kwargs.get("attention_mask", None)

                if attention_mask is not None and torch.is_tensor(attention_mask):
                    bias = self.build_attention_phase_bias(
                        phase_state=phase_state,
                        mode=mode,
                    ...
                    if bias is not None:
                        ...
                        self._mt_attention_bias_active = True

                return original_forward(*args, **kwargs)

            attn.forward = MethodType(wrapped_forward, attn)

    def clear_attention_phase_bias(self) -> None:
        if not self._mt_original_attn_forwards:
            self._mt_attention_bias_tensor = None
            self._mt_attention_bias_active = False
            return

        for attn, original_forward in self._mt_original_attn_forwards:
    ...
    def run_prompt(self, prompt: str, forced_mode: Optional[str] = None) -> RunResult:
        ...
        if self.use_local_lora and self.lora_ready:
            ...
            response = self.call_local_model(
                system_prompt,
                prompt,
                pv,
                phase_state=phase_state,
        ...
        scores.update({
            ...
            "phase_logits_bias_active": 0.0,
            "phase_attention_bias_active": 1.0 if self._mt_attention_bias_active else 0.0,
            "phase_kv_bias_active": 1.0 if self._mt_kv_bias_active else 0.0,
            "phase_hidden_state_bias_active": 1.0 if self._mt_hidden_state_bias_active else 0.0,
        })
