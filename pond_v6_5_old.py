...
# Safety reset size — change to 64 or 128 if OOM happens again
RESET_GRID_SIZE = 128
...
def text_to_ripple(text, H, W, EMB, strength=0.10):
    ...
    for g in groups:
        part = seeded_blob(f"group::{g}", H, W, EMB, strength=0.05, radius=5, channel_step=max(1, EMB // 10))
    ...
    for r in roles:
        part = seeded_blob(f"role::{r}", H, W, EMB, strength=0.07, radius=6, channel_step=max(1, EMB // 12))
    ...
    for tok in salient[:6]:
        part = seeded_blob(f"token::{tok}", H, W, EMB, strength=0.04, radius=3, channel_step=max(1, EMB // 12))
...
class Pond(nn.Module):
    def __init__(self, H=64, W=64, EMB=32, slots=4):
        ...
        self.growth_coh_threshold = 0.35
        self.growth_lock_threshold = 0.35
        self.growth_required_steps = 8
        self.max_grid_size = 1024  # or 2048 if you're feeling wild — pond can ask for as much as it wants
        self.growth_pad = 16

    def reset_grid_size(self, new_size=RESET_GRID_SIZE):
        """Emergency shrink if VRAM gets too tight — keeps memories, just smaller field."""
        if self.H == new_size and self.W == new_size:
            return
        ...
        with torch.no_grad():
            ...
            y0 = (new_size - self.H) // 2
            x0 = (new_size - self.W) // 2
            y0 = max(0, y0)
            x0 = max(0, x0)
    ...
    def salience(self, ripple, z_after, text=None):
        ...
        s = 0.55 * ripple_energy + 0.15 * coh + 0.10 * lock + rep + role_bonus
        return s
    ...
    def write_memory(self, ripple, z_after, text=None):
        ...
        if slot_idx is None:
            slot_idx = self.preferred_slot(text or "")
        if slot_idx is None:
            slot_idx = 0
        ...
        updated = c_add(
            c_scale(mem_i, 1.0 - self.memory_decay),
            c_scale(imprint, self.memory_write * s * role_scale)
        )

        if "time" in detect_groups(text or "") or "assert_time" in detect_roles(text or ""):
            self.slot_energy[slot_idx] += 0.5  # Boost energy for time slot so newest day wins
    ...
    def decode_slot(self, slot_idx, text):
        ...
        for key, anchor_text in ANCHOR_TEXTS.items():
            ...
            sc = overlap_score(c_normalize(mem_i), c_normalize(anchor)).item()
            scores[key] = sc
        ...
        if "query_time" in roles:
            if slot_idx == 3:
                if scores["time_thursday"] > 0.15:
                    return "I think today is Thursday."
                if scores["time_tuesday"] > 0.15:
                    return "I think today is Tuesday."

        return None

    def check_and_grow(self):
        """MT Law: expand only when sustained excellent coherence demands more room."""
        ...
        # Lower coh & lock = BETTER — so increase stability when excellent
        if coh < 0.15 and lock < 0.15:  # Tightened from 0.35 — only grow on very good coherence
    ...
    def load(self):
        if os.path.exists(self.save_path):
            ...
            saved_H = int(state.get("H", self.H))
            saved_W = int(state.get("W", self.W))
