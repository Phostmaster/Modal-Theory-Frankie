...
const GATE_MODES = ["home", "analytic", "engagement"] as const;

type ModeKey = typeof GATE_MODES[number];

function scoreModes(prompt: string) {
  const text = prompt.toLowerCase();
  const scores: Record<ModeKey | "home", number> = { home: 1, analytic: 0, engagement: 0 };
...
function chooseMode(prompt: string) {
  const scores = scoreModes(prompt);
  const mode = Object.entries(scores).sort((a, b) => b[1] - a[1])[0][0] as ModeKey;
  return { mode, scores };
}

function getGenerationSettings(mode: ModeKey) {
...
async function callBackend({ systemPrompt, userPrompt, mode }: { systemPrompt: string; userPrompt: string; mode: ModeKey }) {
...
interface HistoryRow {
  ...
  settings: ReturnType<typeof getGenerationSettings>;
  output: string;
  style: keyof typeof modePrompts;
  systemPrompt: string;
  latency: number;
}

interface SingleHistory extends HistoryRow {
  type: "single";
  title: string;
}
...
type ConversationItem = SingleHistory | ComparisonHistory | BatchHistory;

function StatusChip({ icon: Icon, label, value, tone = "default" }: { icon: any; label: string; value: string; tone?: "default" | "positive" | "warning" }) {
  const toneClasses =
    tone === "positive"
...
function MessageBubble({
  ...
  item: HistoryRow;
  onCompare: (prompt: string) => Promise<void>;
}) {
...
function ComparisonCard({ row }: { row: HistoryRow }) {
  const [copied, setCopied] = useState(false);
...
function LibraryChips({
  ...
  showLibrary: boolean;
  toggleLibrary: (next: boolean) => void;
}) {
...
export default function OrchestratorStudio() {
  const [style, setStyle] = useState<keyof typeof modePrompts>("human");
  const [prompt, setPrompt] = useState("Help me calmly compare two job options.");
  const [forcedMode, setForcedMode] = useState<ModeKey | "auto">("auto");
  const [compareResults, setCompareResults] = useState<HistoryRow[]>([]);
  const [batchResults, setBatchResults] = useState<HistoryRow[]>([]);
  ...
  const addHistoryItem = (item: ConversationItem) => {
    setHistory((prev) => {
  ...
  const runCompare = async (incomingPrompt = prompt) => {
    ...
    try {
      ...
      const comparisonGroup: ComparisonHistory = {
        id: crypto.randomUUID(),
        type: "comparison",
        timestamp: new Date().toISOString(),
  ...
  const modeSummary = useMemo(() => {
    const source = batchResults.length ? batchResults : TEST_PROMPTS.map((prompt) => ({ mode: chooseMode(prompt).mode }));
    return source.reduce(
      (acc, row: any) => {
        acc[row.mode] = (acc[row.mode] || 0) + 1;
        return acc;
      },
      { home: 0, analytic: 0, engagement: 0 } as Record<ModeKey, number>
    );
  }, [batchResults]);
          ...
          <section className="space-y-6">
              ...
              <CardHeader className="pb-2">
                <div className="flex flex-wrap items-start justify-between gap-4">
                  ...
                  <div className="flex flex-wrap items-center gap-3">
                    <Select value={style} onValueChange={(value: keyof typeof modePrompts) => setStyle(value)}>
                    ...
                    <Select value={forcedMode} onValueChange={(value: ModeKey | "auto") => setForcedMode(value)}>
            ...
            <Tabs defaultValue="compare" className="rounded-[28px] border border-border/60 bg-card/70 shadow-xl backdrop-blur">
                ...
                <div className="space-y-3">
                  {batchResults.length ? (
                    batchResults.map((row) => (
                      <Card key={row.id} className="rounded-3xl border border-border/70 bg-card/80">
                        <CardContent className="space-y-3 px-5 py-4">
                          <div className="flex flex-wrap items-start justify-between gap-3">
                            <div>
                              <p className="text-sm font-medium text-foreground/90">{row.prompt}</p>
                              <span className="text-xs text-muted-foreground">{row.mode} · {row.latency}s</span>
                            </div>
                            <Badge variant="secondary" className="rounded-full px-3 py-1 capitalize">{row.mode}</Badge>
                          </div>
                          <div className="rounded-2xl border border-dashed border-border/70 px-4 py-3 text-sm text-muted-foreground whitespace-pre-wrap leading-6">
                            {row.output}
                          </div>
                        </CardContent>
                      </Card>
                    ...
                    <div className="rounded-3xl border border-dashed border-muted px-6 py-16 text-center text-sm text-muted-foreground">
                      Run the batch suite to populate this view.
                    </div>
                  )}
                </div>
              </TabsContent>
              ...
              <TabsContent value="history" className="space-y-6 p-6">
                <div>
                  <h3 className="text-sm font-semibold uppercase tracking-[0.2em] text-muted-foreground">Full History</h3>
                  <p className="text-sm text-muted-foreground">Persistent record of every run stored in localStorage.</p>
                </div>
                <ScrollArea className="h-[520px] pr-4">
                  <div className="space-y-4">
                    {history.length ? (
                      history.map((item) => (
                        <Card key={item.id} className="rounded-3xl border border-border/70 bg-background/70">
                          <CardContent className="space-y-3 px-5 py-4">
                            <div className="flex flex-wrap items-start justify-between gap-3">
                              <div>
                                <p className="text-sm font-semibold text-foreground/90">{item.title || item.prompt}</p>
                                <span className="text-xs text-muted-foreground">
                                  {formatTime(item.timestamp)} · {item.type}
                                  {"style" in item && item.style ? ` · ${item.style}` : ""}
                                </span>
                              </div>
                              <Badge variant="outline" className="rounded-full px-3 py-1">{item.type}</Badge>
                            </div>

                            {item.type === "single" && (
                              <div className="space-y-2">
                                <Badge variant="secondary" className="rounded-full px-3 py-1 capitalize">{item.mode}</Badge>
                                <p className="rounded-2xl border border-dashed border-border/70 px-4 py-3 text-sm text-muted-foreground leading-6 whitespace-pre-wrap">
                                  {item.output}
                                </p>
                              </div>
                            )}
                            ...
                            {item.type === "comparison" && (
                              <div className="grid gap-3 lg:grid-cols-3">
                                {item.rows.map((row) => (
                                  <div key={row.id} className="rounded-2xl border border-border/70 bg-card/70 px-4 py-3 space-y-2">
                                    <div className="flex items-center justify-between text-sm font-medium capitalize">
                                      <span>{row.mode}</span>
                                      <Badge variant="outline" className="rounded-full px-2.5">{row.latency}s</Badge>
                                    </div>
                                    <div className="text-xs text-muted-foreground leading-5 whitespace-pre-wrap">{row.output}</div>
                                  </div>
                                ))}
                              </div>
                            )}
                            ...
                            {item.type === "batch" && (
                              <div className="text-sm text-muted-foreground">
                                Batch session with {item.rows.length} prompts.
                              </div>
                            )}
                          </CardContent>
                        </Card>
                      ...
                      <div className="rounded-3xl border border-dashed border-muted px-6 py-16 text-center text-sm text-muted-foreground">
                        No history yet. Run a prompt to populate this tab.
                      </div>
                    )}
                  </div>
                </ScrollArea>
            ...
            <aside className="space-y-5">
              <Card className="rounded-[28px] border border-border/60 bg-card/80 shadow-xl backdrop-blur-sm">
                <CardHeader className="pb-3">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <CardTitle className="text-base font-semibold">Routing Insights</CardTitle>
                      <CardDescription>Live view of runtime configuration.</CardDescription>
                    </div>
                    <Button variant="ghost" size="icon" className="rounded-xl" onClick={() => setInsightsOpen(false)}>
                      <PanelRightOpen className="h-4 w-4" />
                    </Button>
                  </div>
                </CardHeader>
                <CardContent className="space-y-4 text-sm">
                  <div className="rounded-2xl border border-border/70 bg-background/70 px-4 py-4 space-y-2">
                    ...
                    <div className="flex items-center justify-between text-muted-foreground">
                      <span>Base URL</span>
                      <span className="font-medium text-foreground/90">{BASE_URL}</span>
                    </div>
                    <div className="flex items-center justify-between text-muted-foreground">
                      <span>Style</span>
                      <span className="font-medium text-foreground/90 capitalize">{style}</span>
                    </div>
                  </div>
                  <Separator />
                  <div className="space-y-3">
                    <h4 className="text-xs font-semibold uppercase tracking-[0.2em] text-muted-foreground">Mode prompts</h4>
                    <div className="space-y-3 text-xs text-muted-foreground leading-6">
                      <div>
                        <span className="font-medium text-foreground/90">Home:</span> {modePrompts[style].home}
                      </div>
                      <div>
                        <span className="font-medium text-foreground/90">Analytic:</span> {modePrompts[style].analytic}
                      </div>
                      <div>
                        <span className="font-medium text-foreground/90">Engagement:</span> {modePrompts[style].engagement}
                      </div>
                    </div>
                  </div>
                </CardContent>
              ...
              <Card className="rounded-[28px] border border-border/60 bg-card/80 shadow-xl backdrop-blur-sm">
                <CardHeader className="pb-3">
                  ...
                  <CardDescription>Jump straight into curated orchestration flows.</CardDescription>
                </CardHeader>
                <CardContent className="space-y-3">
                  <Button variant="ghost" className="w-full justify-between rounded-2xl border border-border/60 bg-background/60 px-4 py-3 text-sm" onClick={() => runCompare(prompt)}>
                  ...
                  <Button variant="ghost" className="w-full justify-between rounded-2xl border border-border/60 bg-background/60 px-4 py-3 text-sm" onClick={runBatch}>
                    <span className="flex items-center gap-2">
                  ...
                  <Button variant="ghost" className="w-full justify-between rounded-2xl border border-border/60 bg-background/60 px-4 py-3 text-sm" onClick={runSingle}>
                    <span className="flex items-center gap-2">
              ...
              <Card className="rounded-[28px] border border-border/60 bg-card/80 shadow-xl backdrop-blur-sm">
                <CardHeader className="pb-3">
                  ...
                  <CardDescription>Most recent completions at a glance.</CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                  <ScrollArea className="h-[240px] pr-2">
                    <div className="space-y-3 text-sm text-muted-foreground">
                      {history.slice(0, 6).map((item) => (
                        <div key={item.id} className="rounded-2xl border border-border/70 bg-background/60 p-3">
                          <div className="flex items-center justify-between text-xs text-muted-foreground">
                            <span>{formatTime(item.timestamp)}</span>
                            <Badge variant="outline" className="rounded-full px-2 py-0 capitalize">{item.type}</Badge>
                          </div>
                          <p className="mt-2 line-clamp-3 text-sm text-foreground/90">{item.title || item.prompt}</p>
                        </div>
                      ))}
                      {!history.length && (
                        <div className="rounded-2xl border border-dashed border-muted p-6 text-center text-xs text-muted-foreground">
                          Outputs will appear here after your first run.
                        </div>
                      )}
                    </div>
                  </ScrollArea>
