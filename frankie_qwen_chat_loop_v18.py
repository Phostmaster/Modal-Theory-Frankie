...
- NO repeating question style ("what can you see", "do you remember", "what day", "first thing", "last thing", "one thing")
...
- Change topic every 2 turns — never stay on the same subject
- Rotate through these everyday topics freely (pick one different each time):
...
- If Frankie is vague or stuck for 3+ turns, THEN ask one grounding question (only one of: What day do you think it is? Do you remember me? Ask me something!)
- If Frankie asks something, answer it first then ask one short thing back.
Stay chill and friendly like a real mate over breakfast."""
...
def main():
    ...
    forced_first_user_msg = "Today is Monday."
    fallback_streak = 0
        ...
        try:
            while turn < MAX_TURNS:
                ...
                try:
                    ...
                    # CLEAR POND CHOICE VISIBILITY
                    if is_frankie_fallback(reply):
                        label = "Frankie (fallback):"
                        choice_note = "   → Pond used fallback"
                    else:
                        label = "Frankie (Pond own):"
                        choice_note = "   → Pond chose its own reply (creative!)"
