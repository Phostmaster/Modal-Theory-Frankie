...
{{"memories": [{{"summary": "...", "score": 9, "reason": "..."}}, ...]}}"""
    ...
    response = bridge.backend.generate(prompt, temperature=0.3, max_tokens=400) if hasattr(bridge.backend, 'generate') else bridge.respond(prompt, None, None)[0]
    try:
            ...
            while turn < MAX_TURNS:
                ...
                try:
                    ...
                    if image_path:
                        ...
                        last_vision_desc = vision_desc
                        print(f"Vision desc: {vision_desc}")
                        log.write(f"[{time.strftime('%H:%M:%S')}] Vision desc: {vision_desc}\n")

                    # === TYPOS + ULTRA-STRONG RECALL + FORCED ANSWER ===
                    final_text = user_text
                    keywords = ["colour", "color", "colur", "coler", "colr", "item", "name", "furniture", "sofa", "table", "chandelier", "room", "carrot", "crrot", "carot", "rose"]
                    if last_vision_desc and any(k in user_text.lower() for k in keywords):
                        final_text = f"""You just saw this exact image: {last_vision_desc}

Peter asked: {user_text}

RULES YOU MUST FOLLOW:
- Answer in ONE short sentence only
- Be 100% factual using only the image details
- Start your answer with the fact (e.g. "The carrot is orange")
- NEVER say "I'm here and listening", "hmm", or ask questions back
- Do not be poetic or vague"""
                        print("🔄 Recall boost triggered — forcing direct factual answer")

                    reply, state, intention, scored = bridge.respond(
                        final_text, image_path=image_path, vision_desc=vision_desc
                    )

                    # === FALLBACK OVERRIDE (safe version) ===
                    if is_frankie_fallback(reply) and any(k in user_text.lower() for k in keywords):
                        print("⚡ Fallback override triggered — forcing direct answer")
                        force_prompt = f"""From this image: {last_vision_desc}
Answer Peter's question '{user_text}' in one short factual sentence only. Start with the fact."""
                        reply, _, _, _ = bridge.respond(force_prompt, image_path=None, vision_desc=last_vision_desc)
