...
# ============================================================
# FRANKIE CHANNELS TOY — INTEGRATED v3
# Fresh-start stable base + minimal logging
# ============================================================
...
# ---------- Auto-run control ----------
NUM_AUTO_SESSIONS = 1      # change this number to how many full sessions you want to run automatically
TURNS_PER_SESSION = 25    # turns per waking session (you wanted 100)
...
DRIFT_HISTORY_PATH = "frankie_drift_history.json"
...
def topo_pairs_dict(state):
    return {
        ...
        "qs_vs_dc": round(topo_pair_value(state, QUIET_SETTLE, DREAM_CONSOLIDATION), 3),
        "cq_vs_pe": round(topo_pair_value(state, COGNITIVE_QUERY, PRIMARY_ENGAGEMENT), 3),
        "cq_vs_dc": round(topo_pair_value(state, COGNITIVE_QUERY, DREAM_CONSOLIDATION), 3),
        "pe_vs_dc": round(topo_pair_value(state, PRIMARY_ENGAGEMENT, DREAM_CONSOLIDATION), 3),
    }
...
def update_drift_history(state):
    ...
    entry = {
        ...
        "topo_pairs": topo_pairs_dict(state)
    }
    history.append(entry)
    history = history[-30:]   # keep last 30 sessions only
...
def make_fixed_templates():
    ...
    # 1) Cognitive/Query
    g1 = np.exp(
        -((x - (cx + 10)) ** 2 + (y - (cy - 8)) ** 2) / (2.0 * 5.5**2)
    ).astype(np.float32)
    templates_re[COGNITIVE_QUERY] = g1 * np.cos(1.45)
    templates_im[COGNITIVE_QUERY] = g1 * np.sin(1.45)
...
def dream_session(state, run_id, turn_rows, start_index=2000):
    print("────────────────────────────────────────────────────────────")
...
if __name__ == "__main__":
    ...
    print("Frankie Channels Toy — Integrated v3 (Auto Mode)\n")
    ...
    for session_num in range(1, NUM_AUTO_SESSIONS + 1):
        ...
        for i, text in enumerate(waking_inputs * (TURNS_PER_SESSION // 6 + 1), start=1):
        ...
        # ---------------- Dream ----------------
        dream_session(state, run_id, turn_rows)
        ...
        # Auto-close the plot window so the loop continues
        plt.close('all')
