...
ENABLE_VISION_OVERALL = True          # Master switch: Set False for maximum text-only speed
VISION_AGGRESSIVENESS = 0.5           # 0.0 = very lazy (skip vision a lot), 1.0 = eager
VISION_TIMEOUT = 25                   # Seconds to wait for vision model (was 60)
MAX_VISION_WORDS = 35                 # Limit vision description length for speed

ROLE_FIT_BOOST = 1.65                 # Increase this if Frankie forgets who you are
MEMORY_RESONANCE_BOOST = 1.45         # Increase if he forgets past context
SAFE_FALLBACK_PENALTY = 0.75          # Penalty for the boring "I'm here and listening" line
VISION_SKIP_THRESHOLD = 0.4           # Higher = skip vision more aggressively in casual chat
...
class FrankieLLMBridge:
    ...
    def should_use_vision(self, user_text: str, image_path: Optional[str], mode: str) -> bool:
        """Frankie decides internally whether vision is worth running right now."""
        if not ENABLE_VISION_OVERALL:
            return False
        if image_path and os.path.exists(image_path):
            return True  # User actually gave an image → always try

        # Internal smart decision
        text_lower = user_text.lower()
        visual_words = ["see", "look", "image", "picture", "photo", "describe", "what is this", "show"]
        
        is_visual_question = any(word in text_lower for word in visual_words)
        is_simple_mode = mode in ["greeting_reply", "open_reply", "identity_self_mark", 
                                  "identity_other_mark", "time_reply", "memory_reply"]

        # Use aggressiveness knob to decide in borderline cases
        if is_visual_question:
            return True
        if is_simple_mode:
            return False  # Skip vision in casual chat

        # Random-ish decision based on aggressiveness for other cases
        return random.random() < VISION_AGGRESSIVENESS
    ...
    def build_user_ripple(
        ...
        """Build ripple — vision only when actually useful."""
        ripple = text_to_ripple(user_text, self.pond.H, self.pond.W, self.pond.EMB)
        resolved_vision_desc = vision_desc

        # === Gentle lazy vision decision ===
        text_lower = user_text.lower()
        needs_vision = bool(image_path and os.path.exists(image_path))

        if not needs_vision:
            # Skip vision for normal chat unless user clearly asks about an image
            if not any(word in text_lower for word in ["see", "look", "image", "picture", "photo", "describe", "what is this"]):
                return ripple, None   # Fast path — no vision

        # Only reach here if we really need vision
        if image_path and not resolved_vision_desc:
    ...
    def score_candidate(self, state: FrankieState, candidate_text: str) -> CandidateScore:
        ...
        # === Gentle boosts using the new knobs (Snip 2) ===
        adjusted_role_fit = role_fit * ROLE_FIT_BOOST
        adjusted_memory_resonance = memory_resonance * MEMORY_RESONANCE_BOOST

        # Very light penalty so "I'm here and listening" doesn't win so easily
        if "i'm here and listening" in candidate_text.lower().strip():
            adjusted_role_fit -= SAFE_FALLBACK_PENALTY
            adjusted_memory_resonance -= SAFE_FALLBACK_PENALTY * 0.6

        # Final total — everything else stays exactly as before
        total = (
            W_LOCK_ALIGNMENT    * lock_alignment +
            W_MEMORY_RESONANCE  * adjusted_memory_resonance +
            W_COHERENCE         * coherence_score +
            W_SALIENCE          * salience_score +
            W_REPETITION        * repetition_affinity +
            W_ROLE_FIT          * adjusted_role_fit +
