...
class SessionMetrics:
    """Tracks per-session statistics for trend analysis."""
    session_start: str = field(default_factory=lambda: datetime.now().isoformat())
    total_turns: int = 0

    # Choice origin
    frankie_native_wins: int = 0       # Frankie's seed beat all LLM candidates
    llm_wins: int = 0                  # LLM candidate beat Frankie's seed

    # Fallbacks
    oom_fallbacks: int = 0             # OOM resets triggered
    empty_candidate_fallbacks: int = 0 # No valid candidates after filtering

    # Question detection
    questions_asked: int = 0           # Frankie's chosen reply contains a question

    # Score tracking (running totals for averages)
    ...
    # Per-turn log (full detail for CSV export)
    ...
    def record_turn(
        ...
        # Question detection — did Frankie ask something?
        if "?" in reply:
...
class EchoBackend(BaseBackend):
    """Fallback backend — no LLM required, returns fixed phrases."""
    def generate_phrasings(self, prompt: str) -> List[str]:
        return [
...
class FrankieLLMBridge:
    ...
    def get_vision_description(self, image_path: str) -> Optional[str]:
        ...
        try:
            ...
            # Clean up common artefacts from vision models
    ...
    def score_candidate(self, state: FrankieState, candidate_text: str) -> CandidateScore:
        ...
        # Memory resonance — FIX: single clean assignment path
        ...
        # Vision match — FIX: single clean init, no double assignment
        vision_match = 0.0
        if state.vision_desc:
            ...
            if vision_keywords:
                ...
                if vision_match >= 0.4:
                    role_fit += 0.6  # bonus for grounding in what Frankie saw
                    role_fit = min(role_fit, 1.0)
    ...
    def respond(
        ...
        try:
            ...
            # Re-process after recovery — OOM flag passed through
            state = self.process_user(user_text, image_path=image_path, vision_desc=vision_desc)
            intention = self.build_intention_packet(state)
...
def print_turn_metrics(metrics: SessionMetrics):
    """Print a one-liner metrics update after each turn."""
