...
HISTORY_REGIMES = [
    {"family": "all_qs_control_v1", "mode": "single_family", "prompt_family": "qs_control"},
    {"family": "all_cq_probe_v2",  "mode": "single_family", "prompt_family": "cq_probe"},
    {"family": "all_pe_probe_v2",  "mode": "single_family", "prompt_family": "pe_probe"},
]

PHASE_LOCK_SWEEP = [1.00, 0.85, 0.70, 0.55, 0.40]
...
ENGAGEMENT_WORDS = {
    "listen_to", "be_with", "stay_with", "hold_space", "comfort", "support",
    "accompany", "join", "remain", "offer", "connect_gently", "meet_gently"
}
...
TEST_LABEL = f"cq_template_1p08_lock_{str(LAMBDA_LOCK).replace('.', 'p')}_v1"
...
def frankie_turn(state, delta_re, delta_im, gain_scale=1.0, is_dream=False):
    ...
    for _ in range(N_RELAX_STEPS):
        ...
        if USE_PHASE_LOCK:
            # LAMBDA_LOCK: 0.0 = no phase pull, 1.0 = full pull to THETA_STAR
...
if __name__ == "__main__":
    ...
    print(f"[Mode] LAMBDA_LOCK = {LAMBDA_LOCK:.2f}")
