...
QWEN_SYSTEM_PROMPT = """You are texting your normal penguin mate Frankie.
Keep every reply short, warm and human.
Rules:
- ONE short sentence + ONE short question (max 24 words total)
- NO brackets, NO extra commentary
- You are ONLY chatting with Frankie the penguin — never invent other people
- Vary topics naturally (no repeating coffee, toast, birds, weather, walks, shops, murals, waking up)
- Never keep asking “what's the first thing” or “what color” questions
- If Frankie is vague, ask one playful grounding question like:
  - What day do you think it is?
  - Do you remember me?
  - What can you see right now?
  - Ask me something!
- If Frankie asks something, answer it first then ask one thing back.
...
def looks_repetitive_qwen(text: str) -> bool:
    t = text.lower()
    repetitive_bits = ["coffee", "toast", "bird", "morning", "walk", "afternoon", "window", "weather", "chair", "booth", "mural", "shop", "woke up", "wake up", "first thing"]
    hard_patterns = ["what's the first thing", "when you woke up", "what color was"]
...
def extract_theme_flags(text: str) -> Set[str]:
    ...
    theme_map = {
        ...
        "chair": ["chair", "booth", "table", "cafe"],
        "bird": ["bird", "birds", "window"],
        "weather": ["sun", "weather", "warm", "rain", "wind"],
        "wake": ["wake", "woke", "waking", "first thing"],
        "memory": ["remember", "memory", "before"],
        "identity": ["name", "frankie", "peter"],
        "day": ["sunday", "monday", "tuesday", "wednesday", "thursday", "friday", "saturday", "day"],
        "spark": ["spark", "light", "hums", "glow"],
        "place": ["shop", "street", "wall", "mural"],
        "color": ["color", "colour", "blue", "green", "red"],
    }
...
def build_qwen_prompt(last_frankie: str, recent_history: List[str], fallback_streak: int, recent_themes: List[Set[str]]) -> str:
    ...
    if frankie_asked_question(last_frankie):
        return f"Recent chat:\n{recent_history_text}\n\nFrankie said: \"{last_frankie}\"\n\nFrankie asked you something.\nAnswer directly in 1 short sentence then ask one short follow-up.\n{avoid_text}Max 24 words. No brackets."
    if fallback_streak >= 4:
        return f"Recent chat:\n{recent_history_text}\n\nFrankie said: \"{last_frankie}\"\n\nFrankie is stuck.\nAsk exactly ONE playful grounding question:\n- What day do you think it is?\n- Do you remember me?\n- What can you see right now?\n- Ask me something!\n{avoid_text}Max 18 words. No brackets."
    if is_frankie_fallback(last_frankie):
        return f"Recent chat:\n{recent_history_text}\n\nFrankie said: \"{last_frankie}\"\n\nFrankie seems vague.\nAsk one short playful grounding question about name, day, or what he sees right now.\n{avoid_text}Max 18 words. No brackets."
    return f"Recent chat:\n{recent_history_text}\n\nFrankie said: \"{last_frankie}\"\n\nReply with one short sentence + one short question.\nKeep it concrete, warm, and playful.\n{avoid_text}Max 24 words. No brackets."

def main():
    print("🚀 Starting v11 Qwen-as-user chat with Frankie...")
    print("   (Warm grounding + no random Sarahs!)")
    ...
    with open(LOG_FILE, "a", encoding="utf-8") as log:
        log.write(f"\n=== NEW SESSION v11 - {time.strftime('%Y-%m-%d %H:%M:%S')} ===\n\n")
        turn = 0
        try:
            while turn < MAX_TURNS:
                ...
                if looks_repetitive_qwen(user_msg):
                    retry_prompt = qwen_prompt + "\nAvoid coffee, toast, birds, weather, walks, murals, windows, chairs, morning, waking up, first-thing questions."
                ...
                try:
                    ...
                    log.write(f" [mode={intention.mode} slot={intention.slot} recall={state.pond_out['recall_score']:.3f} decoded={state.decoded or 'none'}]\n")
