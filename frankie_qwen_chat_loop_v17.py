...
- NO brackets, NO emojis, NO poetry words
- HARD BAN: never mention cat, kettle, hoodie, window, fence, garden, stretch, jump, meow, tail, wink, snow, ice, ridge, wind
- Change topic every 3 turns — never repeat the same idea
...
def main():
    print("🚀 Starting v17 Qwen-as-user chat with Frankie...")
    print("   (Clear [genuine] vs [fallback] labels + no loops!)")
    ...
    with open(LOG_FILE, "a", encoding="utf-8") as log:
        log.write(f"\n=== NEW SESSION v17 - {time.strftime('%Y-%m-%d %H:%M:%S')} ===\n\n")
        turn = 0
        try:
