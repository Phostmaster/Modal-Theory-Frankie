...
- NO brackets, NO emojis, NO poetry words like light, breathing, hum, soft, quiet, feel, glow
...
def looks_repetitive_qwen(text: str) -> bool:
    t = text.lower()
    bad_bits = ["first thing", "snow", "berry", "ice", "light", "breathing", "hum", "soft", "quiet", "feel", "glow"]
...
def main():
    print("🚀 Starting v13 Qwen-as-user chat with Frankie...")
    print("   (No poetry words at all — pure normal mate chat!)")
    ...
    with open(LOG_FILE, "a", encoding="utf-8") as log:
        log.write(f"\n=== NEW SESSION v13 - {time.strftime('%Y-%m-%d %H:%M:%S')} ===\n\n")
        turn = 0
        try:
            while turn < MAX_TURNS:
                ...
                if looks_repetitive_qwen(user_msg):
                    retry_prompt = qwen_prompt + "\nNo poetry words, no first thing, no light, breathing, hum, soft, quiet."
